...
<br><br>(For all gateways, describe the environment, availability [clustered; standalone], use [e.g., dedicated to a single gateway; shared by multiple gateways; used by other services, etc.], who will be responsible for monitoring and managing the gateway, and any other notes).
<br><br>(For any standby gateways, describe the required takeover verification intervals and any characteristics that differ from the primary gateway.)
