<img width="1280" height="700" alt="308 - Image - Title - TMDL DAX INFO View tables" src="https://github.com/user-attachments/assets/fedc5ec7-0614-4708-bcdc-d4d232856c0e" />

## DAX INFO Tables

A common way to self-document a Power BI model is though the use of the DAX INFO functions; 4 calculated tables can be created with a single TMDL script that consists of several steps. The example script performs many actions, including:

- creates a *Doc_Tables* table (using the INFO.VIEW.TABLES() DAX function)
- creates a *Doc_Columns* table (using the INFO.VIEW.COLUMNS() DAX function)
- creates a *Doc_Relationships* table (using the INFO.VIEW.RELATIONSHIPS() DAX function)
- creates a *Doc_Measures* table (using the INFO.VIEW.MEASURES() DAX function)
- creates a *relationship* between the *Doc_Tables* and *Doc_Columns* tables
...
createOrReplace
	...
	/// Use Data/Model to select the desired tables and relationships (use CTRL to individually select them in the order you want them to appear in the TMDL script, then drag all to the TMDL view)
	table Doc_Tables
		...
		partition Doc_Tables = calculated
			mode: import
			source = ```
					INFO.VIEW.TABLES()
				```

		annotation PBI_Id = 6938528295a54753a36acf18da149dee
	...
	table Doc_Columns
		...
		partition Doc_Columns = calculated
			mode: import
			source = INFO.VIEW.COLUMNS()

		annotation PBI_Id = 99d4a07d33eb4e8fb7930de66b118c12
	...
	table Doc_Relationships
		...
		partition Doc_Relationships = calculated
			mode: import
			source = INFO.VIEW.RELATIONSHIPS()

		annotation PBI_Id = 278678e5cd514630bc95de80124491ea
	...
	table Doc_Measures
		...
		partition Doc_Measures = calculated
			mode: import
			source = ```
					INFO.VIEW.MEASURES()
				```

		annotation PBI_Id = bde1d28a16514c4fa86c35a946c0298c

	relationship 8356ae8c-adbe-a4b9-4c87-93196143e3eb
		fromColumn: Doc_Columns.Table
		toColumn: Doc_Tables.Name
```
</details>
